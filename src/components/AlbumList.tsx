...
const AlbumList: React.FC<AlbumListProps> = ({ onViewAlbum }) => {
  ...
  const handleCreateAlbum = async (e: React.FormEvent) => {
    ...
    // Buscar o evento selecionado para gerar o nome automaticamente
    ...
    const sessionTypeLabel = selectedEvent.session_type ? 
      sessionTypeLabels[selectedEvent.session_type] || selectedEvent.session_type : 
      'Sessão';
    const autoGeneratedName = `${sessionTypeLabel} - ${selectedEvent.client_name}`;
            ...
            {selectedEventId && (
              <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <p className="text-sm text-blue-800">
                  <strong>Nome da sessão será:</strong>
                </p>
                <p className="text-blue-900 font-medium mt-1">
                  {(() => {
                    const selectedEvent = events.find(event => event.id === selectedEventId);
                    if (!selectedEvent) return 'Evento não encontrado';
                    
                    const sessionTypeLabel = selectedEvent.session_type ? 
                      sessionTypeLabels[selectedEvent.session_type] || selectedEvent.session_type : 
                      'Sessão';
                    return `${sessionTypeLabel} - ${selectedEvent.client_name}`;
                  })()}
                </p>
              </div>
            )}
